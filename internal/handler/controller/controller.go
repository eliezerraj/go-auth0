...
func (h *HttpWorkerAdapter) OAUTHCredential(rw http.ResponseWriter, req *http.Request) error {
	...
	span := lib.Span(req.Context(), "controller.OAUTHCredential")
	defer span.End()
...
func (h *HttpWorkerAdapter) OAUTHCredentialRSA(rw http.ResponseWriter, req *http.Request) error {
	...
	span := lib.Span(req.Context(), "controller.OAUTHCredentialRSA")
	defer span.End()
	...
	res, err := h.workerService.OAUTHCredentialRSA(req.Context(), credential)
	if err != nil {
		var apiError APIError
		switch err {
			case erro.ErrStatusUnauthorized:
...
func (h *HttpWorkerAdapter) TokenValidation(rw http.ResponseWriter, req *http.Request) error {
	...
	span := lib.Span(req.Context(), "controller.TokenValidation")
	defer span.End()
...
func (h *HttpWorkerAdapter) WellKnown(rw http.ResponseWriter, req *http.Request) error {
	...
	span := lib.Span(req.Context(), "controller.WellKnown")
	defer span.End()

	var jwks core.Credential
	vars := mux.Vars(req)
	varJwks := vars["jkws"]
	jwks.Token = varJwks
...
func (h *HttpWorkerAdapter) TokenValidationRSA(rw http.ResponseWriter, req *http.Request) error {
	...
	span := lib.Span(req.Context(), "handler.TokenValidationRSA")
	defer span.End()

	jwksData := core.JwksData{}
	...
	jwksData.Token = bearerToken

	res, err := h.workerService.TokenValidationRSA(req.Context(), jwksData)
	if err != nil {
		var apiError APIError
		switch err {
		case erro.ErrStatusUnauthorized:
